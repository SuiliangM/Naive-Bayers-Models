    ...
    with open(training_file, "r") as rfp:
        ...
        while line:
            ...
            for feature_pair in list_of_features:
                ...
                training_data_structure[class_name][feature][0] += 1
...
def read_training_data(training_file):
    ...
    second_pass(training_file, training_data_structure, all_features, occurrence_of_class)

    return training_data_structure, all_features, occurrence_of_class    
...
def output_model_file(training_data_structure, model_file, class_prior_delta, cond_prob_delta,
                      all_features, occurrence_of_class):
    with open(model_file, "w") as wfp:
        ...
        for class_name in training_data_structure:    
            ...
            for feature in all_features:
                ...
                prob = float(cond_prob_delta + feature_count) / float(2 * cond_prob_delta + class_count)
...
def get_summation(all_features, training_data_structure, class_name, 
                  list_of_features):
    ...
    for feature in all_features:
        ...
        if feature not in list_of_features: 
            ## get the prob
            prob = training_data_structure[class_name][feature][1]
            prob_complement = 1 - prob
            
            log_prob = math.log10(prob_complement)
            
            sum_to_return_1 = sum_to_return_1 + log_prob
            
        else:
            log_prob = training_data_structure[class_name][feature][2]
            sum_to_return_2 = sum_to_return_2 + log_prob
                                
    return sum_to_return_1 + sum_to_return_2
...
def classify(training_data_structure, all_features, occurrence_of_class, wfp, training_file, type):
    ...
    with open(training_file, "r") as rfp:
        ...
        for i, line in enumerate(tqdm(rfp)):           
            ...
            ## this stores all the features of this training instance
            ...
            for class_name in occurrence_of_class:    
                ...
                summation = get_summation(all_features, training_data_structure, class_name, 
                                              list_of_features)

                log_prob_class = occurrence_of_class[class_name][2]

                log_classify_val = log_prob_class + summation 

                if class_name not in predict:
                    predict[class_name] = log_classify_val
...
def main():
    ...
    training_data_structure, all_features, occurrence_of_class = read_training_data(training_file)
